...
Destruct: -3-4i		// Complex first is popped off the stack after CompareComplex fn. uses it

Destruct: -3-4i 	// main ends and all objects that went on the stack go out of scope 
Destruct: 10+4i	// and therefore get popped off 
Destruct: -3-4i
Destruct: -3+4i
Destruct: 1.5+2i
...
The object is constructed using the keyword “new” which means that it will not go on the stack. Instead memory is allocated for in the heap. For that reason, only the pointer is destroyed when cPtr goes out of scope in line 20 since the keyword “delete” has not been used to delete the object from memory. This object stays in memory until the program ends. 
